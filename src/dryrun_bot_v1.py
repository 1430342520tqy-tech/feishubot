...
# -*- coding: utf-8 -*-
"""
飞书跟单 · dryRun 机器人（纸面模式，按真实规则算单）
- 监控指定飞书群，逐条抓取消息（含真实时间戳：message-id 高位 = Unix 秒）
- 图片：等真实加载后抓取，存 PNG
- 读图：像素定位色块 -> 逐块放大识别 -> 红块=止损，止损上方第一个标签=开仓价，
        开仓价上方的实线横线=止盈（取前3档），实线覆盖率用于剔除坐标数字/当前价虚线
- 出单：300U 保证金 x3 = 900U 名义；3 档止盈各平 1/3；TP1 后止损移开仓价；最多同时 3 笔
- 通知：飞书（webhook 或应用 API），未配置则只写日志
"""
import os, re, json, time, base64, datetime, hashlib, threading, traceback
...
def notify(text):
    ...
    if hook:
        try:
            ...
            if r.get("code") not in (0, None) and r.get("StatusCode") not in (0, None):
                log("   webhook 返回: " + json.dumps(r, ensure_ascii=False)[:160])
        except Exception as e:
            log("   webhook 失败: " + str(e)[:120])
        return
    app = cfg.get("app")
    if app and app.get("app_id") and app.get("receive_id"):
        try:
            h = {"Content-Type": "application/json; charset=utf-8"}
            tk = requests.post("https://open.feishu.cn/open-apis/auth/v3/tenant_access_token/internal",
                               headers=h, data=json.dumps({"app_id": app["app_id"], "app_secret": app["app_secret"]}).encode(), timeout=20).json()
            if tk.get("code") == 0:
                body = {"receive_id": app["receive_id"], "msg_type": "text", "content": json.dumps({"text": text})}
                rr = requests.post("https://open.feishu.cn/open-apis/im/v1/messages?receive_id_type=" + app.get("receive_type", "chat_id"),
                                   headers={"Authorization": "Bearer " + tk["tenant_access_token"], "Content-Type": "application/json; charset=utf-8"},
                                   data=json.dumps(body).encode("utf-8"), timeout=20).json()
                if rr.get("code") != 0:
                    log("   应用发送失败: " + json.dumps(rr, ensure_ascii=False)[:160])
            else:
                log("   token 失败: " + json.dumps(tk, ensure_ascii=False)[:120])
        except Exception as e:
            log("   应用发送异常: " + str(e)[:120])
...
def price_of(coin):
    try:
        t = exchange().fetch_ticker(coin.upper() + "/USDT:USDT")
        return float(t["last"])
    except Exception:
        return None
...
def _ds_vision(prompt, img_path):
    b64 = base64.b64encode(open(img_path, "rb").read()).decode()
    body = {"model": "deepseek-v4-flash-vision-exp", "temperature": 0,
            "messages": [{"role": "system", "content": "You read numbers from crypto chart labels. STRICT JSON only."},
...
def read_chart(path):
    ...
    for t in sorted(merged, key=lambda t: t["y1"]):
        ...
        try:
            v = _ds_vision('Return {"text":"<digits exactly as printed>","value":<number>}', p)
        except Exception:
            continue
...
def parse_text(text):
    prompt = ("从这条加密货币跟单消息里抽取开单信息。只输出JSON："
              ...
              "\"stop\":数字或null,\"targets\":[数字],\"tp_on_chart\":bool,\"market\":\"futures|spot|null\","
              "\"type\":\"open|manage|info\",\"manage_action\":\"close_all|trim|move_stop_to_cost|null\"}"
              " 规则：只使用消息里真实出现的数字，绝不编造；止盈写在图上就设 tp_on_chart=true 且 targets 为空。")
...
# ---------------- 计划与通知 ----------------
def fmt_plan(coin, direction, entry, sl, tps, src, when, note="", add=None, timer=None):
    d = 1 if (direction or "LONG").upper() == "LONG" else -1
    lines = []
    lines.append("【已开单·纸面】%s/USDT 永续 · %s" % (coin, "做多 LONG" if d == 1 else "做空 SHORT"))
    lines.append("来源：%s   信号时间：%s" % (src, when))
    lines.append("金额：保证金 %.0fU × %d倍 = 名义 %.0fU" % (MARGIN, LEV, NOTIONAL))
    if isinstance(entry, (int, float)):
        if add:
            lines.append("入场：头仓 1/3 市价 %.8g ＋ 加仓 2/3 挂 %.8g" % (entry, add))
        else:
            lines.append("入场：市价 %.8g" % entry)
    if isinstance(sl, (int, float)) and isinstance(entry, (int, float)) and entry:
        pct = (sl - entry) / entry * 100 * d
        pnl = (sl - entry) * d / entry * NOTIONAL
        lines.append("止损：%.8g → %.2f%%（不含杠杆）· 约 %+.1fU" % (sl, pct, pnl))
    else:
        lines.append("止损：需人工确认")
    if tps:
        for i, t in enumerate(tps[:TP_TIERS], 1):
            if isinstance(entry, (int, float)) and entry:
                pct = (t - entry) / entry * 100 * d
                pnl = (t - entry) * d / entry * NOTIONAL / len(tps[:TP_TIERS])
                lines.append("止盈%d：%.8g → %+.2f%%（不含杠杆）· 平1/3 · 约 %+.1fU" % (i, t, pct, pnl))
        lines.append("规则：TP1 成交后止损移到开仓价（保本）")
    else:
        lines.append("止盈：图上未读到，暂不挂（等你确认）")
    if note:
        lines.append("备注：" + note)
    if timer:
        lines.append("⏱ 信号→推送 共 %.1fs（发现 %.1fs / 解析 %.1fs / 读图 %.1fs / 下单 %.1fs / 推送 %.1fs）" % (
            timer["total"], timer["detect"], timer["parse"], timer["chart"], timer.get("order", 0.0), timer["push"]))
    return "\n".join(lines)

# ---------------- 页面 JS / 会话切换 ----------------
SCAN_JS = """(name) => {
  // 1) 先找到当前会话的『标题元素』（右侧顶部、含群名）
  let title = null;
  for (const e of document.querySelectorAll('*')) {
    const r = e.getBoundingClientRect();
    if (r.y >= 0 && r.y < 170 && r.x > 600 && r.width > 60 && e.children.length < 8) {
      const t = (e.innerText || '').trim();
      if (t && t.length < 60 && r.y < 90 && t.indexOf(name) <= 3) { title = e; break; }
    }
  }
  if (!title) return [];
  // 2) 从标题往上找到『同一面板内』的消息列表（彻底避免读到别的会话）
  let cur = title, list = null;
  for (let i = 0; i < 14 && cur; i++) {
    const l = cur.querySelector ? cur.querySelector('.list_items') : null;
    if (l && l.querySelectorAll('.messageItem-wrapper').length > 0) { list = l; break; }
...
TITLE_JS = """(name) => {
  ...
  for (const e of els) {
    const r = e.getBoundingClientRect();
    if (r.y >= 0 && r.y < 160 && r.x > 600 && r.width > 60 && e.children.length < 8) {
      const t = (e.innerText || '').trim();
      if (t && t.length < 60 && r.y < 90 && t.indexOf(name) <= 3) return true;
    }
  }
  return false;
}"""
...
HEADER_DUMP_JS = """() => {
  ...
  for (const e of Array.from(document.querySelectorAll('*'))) {
    const r = e.getBoundingClientRect();
    if (r.y >= 0 && r.y < 160 && r.x > 600 && r.width > 40 && e.children.length < 4) {
      const t = (e.innerText || '').trim();
      if (t && t.length < 60) out.push(Math.round(r.y) + ':' + t.split(String.fromCharCode(10)).join(' ').slice(0, 36));
    }
  }
...
def open_group(page, name, tries=3):
    """切换会话：只用会话『标题行』匹配（避免预览文字误命中），并用元素级点击 + 标题校验"""
    for attempt in range(tries):
        try:
            ...
            pass
        time.sleep(2.0)
        if page.evaluate(TITLE_JS, name):
            return True
        # 兜底：Ctrl+K 全局搜索
        try:
            page.keyboard.press("Control+k"); time.sleep(1.5)
            page.keyboard.type(name, delay=80); time.sleep(2.0)
        ...
        if page.evaluate(TITLE_JS, name):
            return True
    try:
        log("   [切换失败] %s 右侧顶部文字: %s" % (name, page.evaluate(HEADER_DUMP_JS)[:180]))
    except Exception:
        pass
    return False
...
def main():
    log("==== dryRun 机器人启动 ====")
    log("监控群: " + " / ".join(GROUPS))
    seen = {}          # group -> set(message_id)
    open_pos = {}      # coin -> trade dict
    last_id = {}       # group -> newest message id
    saved = {}
    try:
        ...
        saved = {}
    for k, v in (saved.get("last") or {}).items():
        try:
            last_id[k] = int(v)
        except Exception:
            pass
    if last_id:
        log("已载入上次进度（重启不丢消息）：" + ", ".join(
            "%s→%s" % (k, datetime.datetime.fromtimestamp(last_id[k] >> 32, CST).strftime("%m-%d %H:%M")) for k in last_id))
        ...
        for g in GROUPS:
            try:
                if not open_group(page, g):
                    log("[%s] 打开失败（标题未确认）" % g); continue
                time.sleep(1.5)
                page.mouse.move(900, 400); page.mouse.wheel(0, 2600); time.sleep(1.5)
                rows = page.evaluate(SCAN_JS, g)
                ids = [int(r["id"]) for r in rows if r.get("id")]
                if ids:
                    last_id[g] = max(last_id.get(g, 0), max(ids))
                    seen[g] = set(str(x) for x in ids)
                    newest = max(ids)
                    tail = (rows[-1].get("text") or "")[:40]
                    log("[%s] 基线最新 %s  id=%s  末条=%s" % (
                        g, datetime.datetime.fromtimestamp(newest >> 32, CST).strftime("%m-%d %H:%M:%S"), newest, tail))
                else:
                    log("[%s] 基线建立失败（没读到消息）" % g)
            except Exception as e:
                log("[%s] 初始化异常 %s" % (g, str(e)[:100]))
        log("==== 开始实时监控 ====")
        BOT_START = 0 if last_id else int(time.time())
        if BOT_START:
            log("只处理 %s 之后的新消息（历史消息一律忽略，避免回放）" % datetime.datetime.fromtimestamp(BOT_START, CST).strftime("%Y-%m-%d %H:%M:%S"))
        else:
            log("按上次进度继续（只处理比上次更新的消息）")
        notify("【跟单机器人】dryRun 已启动（纸面模式，只抓新消息，不会下单）")
        hb = 0
        while True:
            for g in GROUPS:
                try:
                    ...
                    for r in sorted(new, key=lambda r: int(r["id"])):
                        ...
                        if info.get("type") == "manage" or (info.get("manage_action") and info.get("manage_action") != "other"):
                            act = info.get("manage_action")
                            notify("【博主指令】%s\n群：%s  时间：%s\n动作：%s\n原文：%s" % (coin or "?", g, when, act, txt[:200]))
                            continue
                        ...
                        if True:
                            ...
                            t_order = time.time()
                            if entry is None:
                                notify("【信号·待确认】%s\n无法确定入场价，原文：%s" % (coin, txt[:200])); continue
                            if sl is None and not tps:
                                notify("【信号·待确认】%s %s\n只读到入场 %s，止损/止盈没读准，请确认\n原文：%s" % (coin, dirc, entry, txt[:200]))
                                continue
                            if len(open_pos) >= MAX_OPEN and coin not in open_pos:
                                notify("【信号·跳过】%s\n同时持仓已满 %d 笔，按规则跳过" % (coin, MAX_OPEN)); continue
                            tm = {"detect": t_found - t_sig, "img": t_img - t_found, "parse": t_parse - t_img,
                                  "chart": t_chart - t_parse, "order": t_order - t_chart}
                            tm["total"] = time.time() - t_sig
                            tr = {"coin": coin, "dir": dirc, "entry": entry, "add": add, "sl": sl, "tps": tps,
                                  "t_open": when, "group": g, "text": txt[:300], "status": "OPEN",
                                  "chart_imgs": imgs, "timer": tm}
                            open_pos[coin] = tr
                            timing = "⏱ 信号→推送 共 %.1fs（发现 %.1fs / 抓图 %.1fs / 解析 %.1fs / 读图 %.1fs / 下单 %.1fs）" % (
                                tm["total"], tm["detect"], tm["img"], tm["parse"], tm["chart"], tm["order"])
                            t_push0 = time.time()
                            notify(fmt_plan(coin, tr["dir"], entry, sl, tps, g, when,
                                            note=("图上读到止损/止盈" if chart else "文字解析"), add=add, timing=timing))
                            t_push = time.time()
                            tm["push"] = t_push - t_push0
                            tm["total"] = t_push - t_sig
                    ...
                    log("[%s] 轮询异常 %s" % (g, str(e)[:120]))
            # 持仓纸面监控：价格触及止损/止盈就“结单”
            try:
                for coin, tr in list(open_pos.items()):
                    ...
                    hit = None
                    if tr.get("sl") and ((px - tr["sl"]) * d <= 0):
                        hit = ("止损", tr["sl"])
                    elif tr.get("tps") and ((px - tr["tps"][0]) * d >= 0):
                        hit = ("止盈1", tr["tps"][0])
                    if hit:
                        pnl = (hit[1] - tr["entry"]) * d / tr["entry"] * NOTIONAL
                        tr["status"] = "CLOSED"
                        tr["exit"] = hit[1]; tr["exit_why"] = hit[0]; tr["pnl"] = pnl
                        with open(TRADES, "a", encoding="utf-8") as f:
                            f.write(json.dumps(tr, ensure_ascii=False) + "\n")
            ...
            hb += 1
            json.dump({"open": list(open_pos.keys()), "last": {k: v for k, v in last_id.items()},
                       "ts": datetime.datetime.now(CST).strftime("%Y-%m-%d %H:%M:%S")}, open(STATE, "w"), ensure_ascii=False, indent=1)
            if hb % 20 == 0:
