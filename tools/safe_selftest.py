...
⚠️ 排查提示（2026-09-18 实测踩到，别再被这个假警报骗一次）：
    自检里有一部分断言**依赖 DeepSeek 接口**（文字解析、图片判类）。
    如果你用**非生产的 .env** 搭沙箱（例如复制 dev 目录的 .env，密钥与生产不同），
    这些断言会失败，表现为「B16 自检：8 项失败」/「解析自检：3/5 通过」；
    **只把 .env 换成生产的那一份，同一份代码就全绿**（实测：B16 全部通过、解析 5/5）。
    所以看到这两处失败，先确认沙箱用的是不是生产 .env，再谈代码缺陷。
    另外日志里的 `⚠️ 图片判类失败：'choices'` 是自检**故意构造的失败路径**，
    两次运行都出现在同一行 —— 它不是故障，别拿它当线索。
"""
import os
import sys
import json
