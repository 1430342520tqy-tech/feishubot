...
CRITICAL_COOLDOWN_SEC = 1800  # 致命关键字：也改成 30 分钟
# ⚠️ 原来致命项只冷 5 分钟，结果**正好等于巡检周期（5 分钟）→ 每轮必推**（实测被刷 12 条）。
#    现在统一 30 分钟：真正的故障是"事件型"的，出现一次就够，不需要每 5 分钟提醒一次。
...
# ⚠️ 2026-09-15 实测到**自激循环**（我的 bug，用户被刷了 12 条）：
#   巡检告警 → 机器人把自己的告警读进日志（"发现新消息"行里含告警全文）
#   → 巡检在新增行里又匹配到「熔断」→ 再告警 → 每 5 分钟一条，永不停。
#   「熔断」还被设成 5 分钟冷却，正好等于巡检周期，于是每轮必推。
# 修法：**匹配前先跳过"我们自己发出去的内容"的回声行**。
ECHO_MARKS = ("外部巡检告警", "【跟单机器人】", "🩺", "🔴 新增故障行")
