...
7. **整图直读（`chart_reader=llm`）尚未在真实图上量过准确率** —— 默认关闭；
   ...
   - 2026-09-18 已在少许真实图上对比过（SUSHI / 黄金白底图 / LIT），结论：白底图 `llm` 明显强于 `geo`，
     深色图两者接近；同日生产 `runtime_config.json` 已切 `chart_reader=llm` + `chart_llm_reads=2`，
     **代码默认值仍是 `geo`**，改回一个字段即回滚。整批真实图的准确率仍未有基线数字（见第 9 条）
